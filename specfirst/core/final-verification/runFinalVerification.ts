...
import type { BobPatched, BobSessionMissing } from "../bob/types.js";
...
export function runFinalVerification(options: RunFinalVerificationOptions): FinalVerificationResult {
  ...
  const bobPatchRead = readJson<BobPatched | BobSessionMissing>(bobPatchResultPath);
  if (!bobPatchRead.ok) {
  ...
  const bobPatch = bobPatchRead.value;
