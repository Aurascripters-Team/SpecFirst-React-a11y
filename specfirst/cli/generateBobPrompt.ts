...
const baselineRead = readJson<BaselineRedConfirmed>(baselineResultPath);
if (!baselineRead.ok) {
  fail("baseline_result_malformed", "baselineResult.json could not be parsed.", baselineRead.error);
}

const bv = (baselineRead as { ok: true; value: BaselineRedConfirmed }).value;

if ((bv as unknown as { status: string }).status === "skipped") {
...
if (bv.status !== "red-confirmed") {
  fail(
    "baseline_not_red_confirmed",
    `Phase 6 baseline status is '${bv.status}'. Phase 7 only proceeds from 'red-confirmed'.`,
  );
}
...
const lockedSpecRead = readJson<LockedSpecSuccess>(lockedSpecPath);
if (!lockedSpecRead.ok) {
  fail("locked_spec_malformed", "lockedSpec.json could not be parsed.", lockedSpecRead.error);
}

const testGenRead = readJson<TestGenerationGenerated>(testGenPath);
if (!testGenRead.ok) {
  fail("test_gen_malformed", "testGenerationResult.json could not be parsed.", testGenRead.error);
}

const lv = (lockedSpecRead as { ok: true; value: LockedSpecSuccess }).value;
const tg = (testGenRead as { ok: true; value: TestGenerationGenerated }).value;
...
const promptGenResult: BobPromptGenerated = {
  ...
  component: {
    name: lv.component.name,
    path: lv.component.path,
  },
  inputs: {
