...
const RULES_DIR     = path.join(SPECFIRST_DIR, 'rules');
const REGISTRY_PATH = path.join(RULES_DIR, 'registry.json');

// Run ID is passed as a CLI argument: node phase3_loader.js <run-id>
const runId = process.argv[2];
if (!runId) {
    console.error("❌ Usage: node phase3_loader.js <run-id>");
    process.exit(1);
}

const RUN_DIR             = path.join(SPECFIRST_DIR, 'runs', runId);
const CLASSIFICATION_PATH = path.join(RUN_DIR, 'classification.json');
const RESULT_PATH         = path.join(RUN_DIR, 'manifestLoadResult.json');
const SNAPSHOT_PATH       = path.join(RUN_DIR, 'manifestUsed.json');

// ─── Utility: ensure run dir exists before any write ─────────────────────────
...
// ─── Main Phase 3 loader ──────────────────────────────────────────────────────
function loadManifestForClassification() {
    console.log(`\nStarting Phase 3: Manifest Validation...`);
    console.log(`Run ID : ${runId}`);
    console.log(`Run Dir: ${RUN_DIR}\n`);

    // READ: classification.json written by Phase 2
    if (!fs.existsSync(CLASSIFICATION_PATH)) {
        return failPipeline(
            "classification_not_found",
            `classification.json not found at ${CLASSIFICATION_PATH}. Has Phase 2 run for this run-id?`
        );
    }

    let classification;
    try {
        classification = JSON.parse(fs.readFileSync(CLASSIFICATION_PATH, 'utf8'));
    } catch (e) {
        return failPipeline(
            "classification_malformed",
            `classification.json could not be parsed: ${e.message}`
        );
    }

    console.log(`✅ Read classification.json for component: ${classification.component?.name ?? 'unknown'}\n`);

    // GATE 1: Continuation Gate
    // Phase 3 must not proceed unless Phase 2 explicitly said so.
    if (!classification.nextPhase || !classification.nextPhase.canContinue) {
        return failPipeline(
            "classification_blocked",
            "Phase 2 set canContinue=false. Pipeline is stopped."
        );
    }

    // GATE 2: Manifest ID Gate
    // Phase 3 must never guess — ID must be explicitly provided by Phase 2.
    const manifestId = classification.nextPhase.manifestId;
    if (!manifestId) {
        return failPipeline(
            "missing_manifest_id",
            "No manifestId provided in classification.nextPhase. Phase 3 will not guess."
        );
    }
    ...
    // GATE 4: File Gate
    // The versioned local manifest file must physically exist under specfirst/rules/.
    const manifestPath = path.join(RULES_DIR, registryEntry.file);
    ...
    // All seven source-layer sections must exist.
    ...
    // Cannot hand Phase 4 a manifest with no requirements.
    ...
    // Manifest must declare rejectIf — second safety gate duplicating some Phase 2 logic.
    if (
